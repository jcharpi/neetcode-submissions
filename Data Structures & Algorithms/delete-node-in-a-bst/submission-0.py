...
class Solution:
    def deleteNode(self, root: Optional[TreeNode], key: int) -> Optional[TreeNode]:
        # if we find our node to delete has no children, set parent's reference to it to null
        # else, if children, we must recursively restructure to set root to right most child in left subtree
        if not root:
            return None
